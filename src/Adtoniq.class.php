...
class Adtoniq {
  ...
  // Javascript string to inject in the <head> of the user site
	...
	private $_apiVersion = 'https://integration.adtoniq.com/api/v2';

	// Plugin version number
	private $_version = 'Adtoniq-PHP-1.0.0';
  ...
  // Script loading functions
	...
	function __construct($options) {
		...
		$this->_loadScript = isset($options['loadScript']) ? $options['loadScript'] : 'Adtoniq::_loadAdtoniqScript';
		$this->_saveScript = isset($options['saveScript']) ?  $options['saveScript'] : 'Adtoniq::_saveAdtoniqScript';
	...
	private function _loadAdtoniqScript() {
		try {
	...
	private function _saveAdtoniqScript($script) {
		$pid = getmypid();
	...
	private function _getLatestJavaScript($nonce = '') {
		if ($this->_javascript === '' || $nonce !== '') {
			...
			if ($response->success) {
				...
				throw new Exception('Error initializing Adtoniq for PHP.');
				die();
			}
		}
	}
